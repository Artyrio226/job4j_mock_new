...
public class UnbindAction implements Action {
    ...
    public BotApiMethod<Message> handle(Message message) {
        ...
        var text = "Введите логин и пароль через пробел для отвязки аккаунта telegram от платформы CheckDev.";
    ...
    public BotApiMethod<Message> callback(Message message) {
        ...
        if (parts.length == 2) {
            ...
            Object result;
            try {
                result = tgAuthCallWebClint.doPost(URL_AUTH_UNBIND, person).block();
            } catch (Exception e) {
